...
const AllTools = () => {
  return tabs.map((tool) => (
    <section id={tool.label} className="container px-3 mx-auto my-10">
      <div className="w-max-[1320px] text-black-2">
          ...
          {tool.content.map((item) => (
            ...
            >
              <div className="flex gap-3 items-center">
                <div className={`text-font-xl ${tool.iconColor}`}>{item.icon}</div>
                <div className="transition duration-500 text-font-base font-semibold mb-1 group-hover:text-blue-main lg:text-font-xl">
                  {item.title}
                </div>
              </div>
              <div className="text-font-sm leading-5 py-3 lg:text-font-base">{item.description}</div>
        ...
        <div className=" relative my-14 md:my-28 lg:my-40 border-t-2 border-white">
          ...
          >
            ...
            >
              <img
                className={`h-56 md:h-60 lg:h-80 ${tool.id % 2 > 0 ? "" : ""}`}
                src={tool.image}
